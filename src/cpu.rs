...
impl AddressMode {
    pub fn decode(self,cpu : &mut CPU) -> (u16,i32) {
        use AddressMode::*;
        match self {
            Accumulator=>{cpu.pc = cpu.pc.wrapping_add(1); (0,0)},
            Immediate=>{
                let result = (cpu.pc,0);
            ...
            Indirect => {
                ...
                let hi = if (ptr & 0x00FF) == 0x00FF {cpu.bus.read(ptr & 0xFF00) as u16} else {cpu.bus.read(ptr + 1) as u16};
                ((hi << 8) | lo, 0)
            }
            IndirectX => {
                let base = cpu.fetch();
                ...
                let addr = cpu.bus.read(ptr) as u16 | ((cpu.bus.read((ptr + 1) & 0xFF) as u16) << 8);
                (addr,0)
            }
            IndirectY => {
                let ptr = cpu.fetch();
                let base_addr = cpu.bus.read(ptr as u16) as u16 | ((cpu.bus.read(ptr.wrapping_add(1) as u16) as u16) << 8);
                let effective = base_addr.wrapping_add(cpu.y as u16);
...
pub struct CPU {
    ...
    pub pc : u16,
    pub status : u8,
    #[cfg(not(test))] pub bus : Bus,
    #[cfg(test)] pub bus : crate::tests::TestBus,
    pub ir_disable : bool
}
...
impl CPU {
    ...
    fn adc(&mut self, addr_mode : AddressMode, cycles : i32) -> i32 {

        let (addr, extra_cycles) = addr_mode.decode(self);
    ...
    fn sbc(&mut self, addr_mode : AddressMode, cycles : i32) -> i32 {
    ...
    fn sta(&mut self, address_mode : AddressMode, cycles : i32)->i32 {
        ...
        let needs_dummy = address_mode == AddressMode::AbsoluteX || address_mode == AddressMode::AbsoluteY || address_mode == AddressMode::IndirectY;
        let (addr,extra) = address_mode.decode(self);
    ...
    fn lax(&mut self)->i32 {
        let (addr,_) = AddressMode::ZeroPage.decode(self);
